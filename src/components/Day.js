        ...
        <div key={props.key} className="w-[19.5%] p-3" ref={setNodeRef}>
            ...
            <div className="min-h-[30vh] flex flex-col overflow-y-auto h-[70vh] max-h-[70vh] pt-2 px-4 -mx-4 scrollbar-hide" onMouseOver={handleMouseOver} onMouseOut={handleMouseOut}>
                <SortableContext items={props.tasklist} strategy={verticalListSortingStrategy} id={props.date}>
                    {props.tasks.map((task) => {return <Task enableAnimations = {props.enableAnimations} editedTask={props.editedTask} activeId={props.activeId} overlay={false} date={props.date} key={task.id + "-" + props.date} iid={task.id + "/" + props.date} setCompleted={setCompleted} setRepeatWarning={props.setRepeatWarning} toggleEdit={props.toggleEdit} editTask={props.editTask} tasks={props.tasklist} {...task}/>})}
                </SortableContext>
